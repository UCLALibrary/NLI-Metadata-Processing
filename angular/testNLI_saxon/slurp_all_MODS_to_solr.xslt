...
<xsl:stylesheet version="2.0"
  ...
  <xsl:variable name="single_valued_hashset" select="hashset:new()" xmlns:hashset="java:java.util.HashSet" />
  ...
  <xsl:template match="mods:*[(@type='date') or (contains(translate(local-name(), 'D', 'd'), 'date'))][normalize-space(text())]" mode="slurping_MODS">
    ...
    <!-- The method java.util.HashSet.add will return false when the value is
         already in the set. -->
    <xsl:if test="hashset:add($single_valued_hashset, $field_name)" xmlns:hashset="java:java.util.HashSet">
      ...
      <field>
        <xsl:attribute name="name">
          <xsl:choose>
            ...
            <xsl:when test="$suffix='ms' and hashset:add($single_valued_hashset, string($prefix))" xmlns:hashset="java:java.util.HashSet">
